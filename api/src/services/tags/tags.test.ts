...
describe('tags', () => {
  ...
  scenario('creates a tag', async () => {
    const result = await createTag({
      input: { name: 'String3665244', updatedAt: '2024-11-26T18:44:26.840Z' },
    })

    expect(result.name).toEqual('String3665244')
    expect(result.updatedAt).toEqual(new Date('2024-11-26T18:44:26.840Z'))
  })
  ...
  scenario('updates a tag', async (scenario: StandardScenario) => {
    ...
    const result = await updateTag({
      id: original.id,
      input: { name: 'String41982442' },
    })

    expect(result.name).toEqual('String41982442')
  })
